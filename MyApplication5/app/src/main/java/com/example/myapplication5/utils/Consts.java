package com.example.myapplication5.utils;

public interface Consts {
    int VIEW_MODEL_HOME = 0;
    int VIEW_MODEL_DASHBOARD = 1;
    int VIEW_MODEL_NOTIFICATION = 2;
    String VIEW_MODEL_CLASS = "view_model_class";

    String URL_ADDRESS = "http://10.0.2.2:8080/";
    String SERVER_ADDRESS = "http://192.168.0.104:8080/";
    String SERVER_ADDRESS_EMULATOR = "http://10.0.2.2:8080/";
    String URL_BOOKING_ID = "flight?bookingId=";

    int ACTIVITY_RESULT_CREATE_FILE = 1;
    int ACTIVITY_RESULT_PICK_FOLDER = 2;
    int ACTIVITY_RESULT_PICK_VISA_FILE = 3;
    int ACTIVITY_RESULT_PICK_ID_FILE = 4;
    int ACTIVITY_RESULT_PICK_PHOTO_FILE = 5;


}
